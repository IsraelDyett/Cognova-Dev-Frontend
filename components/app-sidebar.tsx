...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const [mainNavigationMenus, setNavigationMenus] = React.useState<any[]>(sidebarData.mainNavigationMenus)
  ...
  return (
    <Sidebar collapsible="icon" {...props}>
      <SidebarHeader>
        <WorkspaceSwitcher />
        {currentNavbar == "bot" && (
          <Link href={`/${workspaceId}/`} className="space-x-4 group hover:text-muted-foreground flex py-2 text-sm w-full  items-center">
