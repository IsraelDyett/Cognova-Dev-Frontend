...
class ChatWidget {
	...
	calculateIframeStyles(botData) {
		...
		return {
			...baseStyles,
			width: this.options.smallMode
				? `calc(${CONFIG.IFRAME.defaultWidth} * 0.6)`
				: CONFIG.IFRAME.defaultWidth,
			maxWidth: this.options.smallMode ? "48vw" : CONFIG.IFRAME.maxDesktopWidth,
			height: this.options.smallMode ? "48vh" : "80vh",
			maxHeight: this.options.smallMode
				? CONFIG.IFRAME.smallMaxHeight
				: CONFIG.IFRAME.maxDesktopHeight,
			borderRadius: this.options.smallMode
				? CONFIG.IFRAME.borderRadius.small
				: CONFIG.IFRAME.borderRadius.default,
	...
	calculateIframePosition(botData) {
		...
		if (
			(botData.configurations?.embedAngle || CONFIG.POSITION.defaultPosition) ===
			"bottom-right"
		) {
	...
	calculateButtonPosition(botData) {
		...
		if (
			(botData.configurations?.embedAngle ||
				CONFIG.POSITION.defaultPosition ||
				CONFIG.POSITION.defaultPosition) === "bottom-right"
		) {
	...
	calculateButtonDimensions() {
		return {
			...
			padding: this.options.smallMode
				? CONFIG.BUTTON.smallPadding
				: CONFIG.BUTTON.defaultPadding,
		};
	}
	...
	createButtonIcon(iconUrl) {
		...
		img.src =
			iconUrl ||
			`https://pub-a345b220415f489cb555bd6733a2e7a9.r2.dev/transparent-chat-icon.png`;
		img.style.width = "100%";
	...
	calculateWelcomeMessageDimensions() {
		return {
			width: this.options.smallMode
				? CONFIG.WELCOME_MESSAGE.smallWidth
				: CONFIG.WELCOME_MESSAGE.defaultWidth,
	...
	createWelcomeMessage(botData) {
		...
		this.typeMessage(
			message,
			botData.configurations?.embedPingMessage ||
				CONFIG.WELCOME_MESSAGE.defaultEmbedPingMessage,
		);
	...
	createCloseButton(element, position = "bottom", fadeInElements = [], thenFadeMeOut = false) {
		...
		closeButton.addEventListener("mouseenter", () => {
			closeButton.style.backgroundColor =
				CONFIG.WELCOME_MESSAGE.animations.closeButton.hoverColor;
		});
