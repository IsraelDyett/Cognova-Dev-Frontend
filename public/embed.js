...
class ChatWidget {
  ...
  calculateIframePosition(botData) {
    ...
    if (
      (botData.configurations?.embedAngle || CONFIG.POSITION.defaultPosition) === "bottom-right"
    ) {
      position.right = `${CONFIG.POSITION.defaultOffset.right + 2.5 || 2.5}rem`;
    } else if (
      (botData.configurations?.embedAngle || CONFIG.POSITION.defaultPosition) === "bottom-left"
    ) {
  ...
  createWelcomeMessage(botData) {
    ...
    this.typeMessage(
      message,
      botData.configurations?.embedPingMessage || CONFIG.WELCOME_MESSAGE.defaultEmbedPingMessage,
    );
    ...
    this.elements.button.onclick = () => {
      if (this.elements.iframe.style.opacity === "1") {
        ...
        if (this.state.isMobile) {
          ...
          this.appendToParent(
            this.createCloseButton(this.elements.iframe, "top", [this.elements.button], true),
          );
        }
      }
    };
