...
export const scraperApi = {
	async fetchUrls(url: string): Promise<string[]> {
		debug(
			...
			"fetchUrls",
			"STORE",
			"app/(workspace)/[workspaceId]/bots/[botId]/sources/components/scraper-store.ts",
		);
		try {
			...
			throw error instanceof ScraperError ? error : new ScraperError("Failed to fetch URLs");
		}
	},
	...
	async scrapeURLs(urls: string[], workspaceId?: string, botId?: string) {
		...
		try {
			...
			const scrapeResults = await fastApi.sources.addSource(
				workspaceId,
				"website",
				urls,
				botId,
