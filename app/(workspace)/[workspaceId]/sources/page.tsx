...
import {
    associateSourceWithBot,
    deassociateSourceFromBot,
} from '../bots/[botId]/actions'
...
import { getWorkspaceBots, getWorkspaceSources } from '@/app/(workspace)/actions'
import { useSourceBots } from './store'

export const SourcesPage = (props: WorkspacePageProps) => {
    ...
    const handleToggleBot = async (sourceId: string, botId: string, currentlyAssociated: boolean) => {
        try {
            ...
            if (result.success) {
                ...
                if (sourcesRes.success) {
                    setSources(sourcesRes.data)
                    toast.success(`Source ${currentlyAssociated ? 'removed from' : 'added to'} bot successfully`)
                }
            } else {
                throw new Error('Failed to update association')
            }
        } catch (error) {
            toast.error(`Failed to ${currentlyAssociated ? 'remove' : 'add'} source ${currentlyAssociated ? 'from' : 'to'} bot`)
        }
    }
                                ...
                                <TableRow key={source.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Dialog>
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    <DialogTitle>Manage Bots for {source.url}</DialogTitle>
                                                    <DialogDescription>
                                                            ...
                                                            <div key={bot.id} className="flex items-center space-x-2">
                                                                <Checkbox
                                                                    ...
                                                                    onCheckedChange={(checked) => {
                                                                        handleToggleBot(source.id, bot.id, isAssociated)
                                                                    }}
                                                                />
                                                                <label
                                                                    htmlFor={`bot-${bot.id}`}
