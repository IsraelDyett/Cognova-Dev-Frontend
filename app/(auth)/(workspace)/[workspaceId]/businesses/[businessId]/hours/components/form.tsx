...
export function HourForm() {
  const { createHour, updateHour, onCloseCrudForm, initialCrudFormData, isOpenCrudForm } = useHourStore();
  ...
  return (
    <Dialog open={isOpenCrudForm} onOpenChange={onCloseCrudForm}>
      <DialogContent size={"2xl"}>
        <DialogHeader>
          <DialogTitle>
            {initialCrudFormData ? 'Edit' : 'Create'} Hour
          </DialogTitle>
          <DialogDescription>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4grid-cols-1 sm:grid-cols-2">
