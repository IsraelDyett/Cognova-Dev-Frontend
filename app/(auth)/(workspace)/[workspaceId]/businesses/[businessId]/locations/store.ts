...
interface BusinessLocationState {
  ...
  createBusinessLocation: (data: Omit<BusinessLocation, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
...
export const useBusinessLocationStore = create<BusinessLocationState>((set) => ({
  ...
  createBusinessLocation: async (data) => {
    ...
    try {
      ...
      if (response.success) {
        set((state) => ({
          businesslocations: [...state.businesslocations, response?.data ?? ({} as BusinessLocation)]
        }));
