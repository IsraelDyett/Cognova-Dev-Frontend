...
export const use${model.name}Store = create<${model.name}State>((set) => ({
  ...
  fetch${model.name}s: async (workspaceId: string) => {
    ...
    try {
      const response = await list${model.name}s({ where: { workspaceId }});
      if (response.success) {
...
export default function ${model.name}Dashboard() {
  ...
  if (!${model.name.toLowerCase()}s.length && !isOpenCrudForm) return <NoStateComponent />;
  ...
  return (
    <div className="flex h-[450px] shrink-0 items-center justify-center rounded-md border border-dashed">
      <div className="mx-auto flex max-w-[420px] flex-col items-center justify-center text-center">
        ...
        <Button asChild>
          <WorkspaceLink href="/${model.name.toLowerCase()}/create">
